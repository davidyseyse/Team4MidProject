 ...
 */
@WebServlet("/jsp/team4/ticket/EventInputSystemProcessServlet1.do")
public class EventInputSystemProcessServlet1 extends HttpServlet {
	...
	private void processAction(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException, ParseException, ClassNotFoundException {
		...
		ExhibitionBean exb = new ExhibitionBean(onSaleDateTimeStamp,offSaleDateTimeStamp,commDate,dueDate,description,cardId);
		try {
			eis.createSQLServerJndiConn();
			eis.addEvent(eb);
			eis.addExhibition(exb);
			//price
