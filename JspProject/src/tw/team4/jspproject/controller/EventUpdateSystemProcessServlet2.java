 ...
 */
@WebServlet("/jsp/team4/ticket/EventUpdateSystemProcessServlet2.do")
public class EventUpdateSystemProcessServlet2 extends HttpServlet {
	...
	private void processAction(HttpServletRequest request, HttpServletResponse response) throws IOException, SQLException, ParseException, ClassNotFoundException {
		...
		AttractionBean ab = new AttractionBean(eventId,onSaleDateTimeStamp,offSaleDateTimeStamp,commDate,dueDate,description);
		try {
			eis.createSQLServerJndiConn();
			eis.updateEvent(eb);
			eis.updateAttraction(ab);
